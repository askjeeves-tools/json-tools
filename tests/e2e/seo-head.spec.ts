...
test("homepage has SEO meta tags and structured data", async ({ page }) => {
	...
	await expect(expander.locator("summary")).toHaveText("Supported JSON conversions");
	await expect(expander.locator("li")).toHaveCount(1);
	expect(await page.content()).toContain("JSON → CSV");
	...
	for (const expanderEl of await seoExpanders.all()) {
		await expect(expanderEl).not.toHaveAttribute("open", "");
	}
	await expect(
	...
	await expect(
		seoExpanders.filter({
			has: page.locator("summary", { hasText: "Supported JSON conversions" }),
		}),
	).toBeVisible();
	await expect(
...
test("static SEO files are served", async ({ request }) => {
	...
	expect(robotsText).toContain("Sitemap: https://json.askjeeves.cc/sitemap-index.xml");
